...
class APIToolAutoScanner:
    """
    AUTOMATIC API Tool Scanner - HOÀN TOÀN ĐỘNG!
    1. Quét app_config.json để tìm tools có type = "api_tool"
    2. Tự động tạo tools cho mỗi API
    3. KHÔNG HARD-CODE gì cả - tất cả từ config!
    """
    
    def __init__(self, config_path: str = None):
        # ✅ TỰ ĐỘNG TÌM ĐƯỜNG DẪN CHÍNH XÁC
        if config_path is None:
            # Tìm đường dẫn tới app_config.json từ vị trí hiện tại
            current_file = Path(__file__)
            # Đi ngược lên 4 cấp: tools -> advisor -> services -> root
    ...
    def _scan_api_configs(self) -> Dict[str, Dict[str, Any]]:
        """Quét tự động tất cả API configurations từ app_config.json"""
        try:
            ...
            for tool in tools:
                # ✅ HOÀN TOÀN ĐỘNG - LẤY TỪ CONFIG!
                tool_type = tool.get("type", "")
                if tool_type == "api_tool":  # ĐÚNG - LẤY TỪ CONFIG!
    ...
    def create_all_api_tools(self) -> List['AutoGeneratedAPITool']:
        ...
        logger.info(f"🎯 Total auto-generated tools: {len(tools)}")
        return tools
    
    def create_specific_api_tool(self, api_name: str) -> Optional['AutoGeneratedAPITool']:
        """Tạo tool cho một API cụ thể"""
...
class AutoGeneratedAPITool(BaseTool):
    """
    API Tool được tạo TỰ ĐỘNG HOÀN TOÀN từ config!
    KHÔNG HARD-CODE gì cả - tất cả từ app_config.json
    """
    
    def __init__(self, api_config: Dict[str, Any], config_path: str = None):
        # ✅ LƯU CONFIG VÀO LOCAL VARIABLES TRƯỚC
        ...
        logger.info(f"🚀 Auto-generated tool '{self.name}' ready for use!")
    ...
    def _auto_generate_all_properties(self):
        ...
        try:
            ...
            self._api_type = self._api_config.get("type", "api_tool")  # ĐÚNG!
            self._api_description = self._api_config.get("description", "API endpoint")
            
            # Tự động trích xuất cấu trúc input
            ...
            # Tự động trích xuất cấu trúc output
            self._output_config = self._api_config.get("output", {})
            
            # Tự động trích xuất curl để lấy URL và method
            self._curl_command = self._api_config.get("curl", "")
            
            logger.debug(f"✅ Properties generated for {self._api_name}")
    ...
    def _generate_tool_name(self) -> str:
        """Tự động tạo tên tool duy nhất"""
        return f"api_{self._api_name}"
    
    def _generate_tool_description(self) -> str:
        """Tự động tạo mô tả chi tiết từ config"""
        description_parts = []
        
        # Mô tả cơ bản từ config
        description_parts.append(f"🔗 {self._api_description}")
        
        # Tự động đếm parameters
        if self._param_config:
            param_count = len(self._param_config)
            description_parts.append(f"📊 Query Parameters: {param_count}")
            
            # Tự động liệt kê key parameters
            key_params = list(self._param_config.keys())[:3]
            if key_params:
                description_parts.append(f"   Key params: {', '.join(key_params)}")
        
        # Tự động đếm body fields
        if self._body_config:
            body_count = len(self._body_config)
            description_parts.append(f" Body Fields: {body_count}")
            
            # Tự động đếm required fields
            required_fields = [name for name, config in self._body_config.items() 
                             if config.get("required", False)]
            if required_fields:
                description_parts.append(f"   Required: {', '.join(required_fields)}")
        
        # Tự động detect HTTP method
        method = self._auto_detect_http_method()
        description_parts.append(f"🌐 Method: {method}")
        
        # Tự động tạo usage examples
        description_parts.append("\n Usage Examples:")
        
        # Tạo example cho query params
        if self._param_config:
            example_param = next(iter(self._param_config.keys()))
            example_value = self._auto_generate_example_value(self._param_config[example_param])
            description_parts.append(f"   Query: {example_param}={example_value}")
        
        # Tạo example cho body
        if self._body_config:
            example_body = next(iter(self._body_config.keys()))
            example_value = self._auto_generate_example_value(self._body_config[example_body])
            description_parts.append(f"   Body: {example_body}: {example_value}")
        
        return "\n".join(description_parts)
    
    def _auto_detect_http_method(self) -> str:
        """Tự động detect HTTP method từ curl command"""
    ...
    def _generate_input_schema(self) -> Type[BaseModel]:
        """Tự động tạo Pydantic input schema hoàn chỉnh"""
        ...
        if self._param_config:
            for param_name, param_def in self._param_config.items():
                ...
                # Tự động tạo field với smart defaults
        ...
        # TỰ ĐỘNG TẠO header override fields
    ...
    def _auto_generate_example(self, field_def: Dict[str, Any]) -> Any:
        """Tự động tạo smart examples dựa trên field definition"""
        field_type = field_def.get("type", "string")
        field_name = field_def.get("name", "field")
        
        # Smart examples dựa trên field name và type
    ...
    def _auto_generate_example_value(self, field_def: Dict[str, Any]) -> Any:
        """Tự động tạo example value cho field"""
    ...
    def _auto_build_request(self, tool_input: Dict[str, Any]) -> Tuple[str, Dict[str, str], Optional[Dict[str, Any]]]:
        """Tự động build HTTP request từ tool input"""
        
        # Tự động trích xuất URL từ curl
        url_match = re.search(r"'([^']+)'", self._curl_command)
        if not url_match:
            raise ValueError("Could not auto-extract URL from curl command")
        
        base_url = url_match.group(1)
        
        # Tự động build query parameters
        query_params = {}
    ...
    async def _auto_execute_api_call(self, url: str, headers: Dict[str, str], body: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """Tự động execute HTTP request với detected method"""
        method = self._auto_detect_http_method()
    ...
    def _auto_format_response(self, response: Dict[str, Any]) -> str:
        """Tự động format API response"""
        if not response.get("success"):
            return f"❌ API call failed: {response.get('error', 'Unknown error')}"
        
        # Tự động format successful response
        output = [
            f"✅ {self._api_name} API call successful!",
...
# Convenience functions cho dễ tích hợp
def create_auto_api_tools(config_path: str = None) -> List[AutoGeneratedAPITool]:
    """Tạo tất cả auto-generated API tools"""
...
def create_auto_api_tool(api_name: str, config_path: str = None) -> Optional[AutoGeneratedAPITool]:
    """Tạo một auto-generated API tool cụ thể"""
    scanner = APIToolAutoScanner(config_path)
...
if __name__ == "__main__":
    async def test_auto_generation():
        """Test hệ thống auto-generation"""
        
        print("🚀 Testing APIToolAutoScanner...")
        print(f"Current working directory: {os.getcwd()}")
        print(f"File location: {__file__}")
        
        try:
            # Tạo tất cả tools tự động
            all_tools = create_auto_api_tools()
            print(f"✅ Auto-generated {len(all_tools)} tools:")
            ...
            for tool in all_tools:
                ...
                if hasattr(tool.args_schema, 'model_fields'):
                    print("📋 Auto-generated Input Fields:")
                    for field_name, field_info in tool.args_schema.model_fields.items():
                        # ✅ SỬA LỖI - SỬ DỤNG annotation thay vì type_
                ...
                # Fallback cho Pydantic V1
                    ...
                    print("📋 Auto-generated Input Fields (Pydantic V1):")
            ...
            if all_tools:
                ...
                print("\n❌ No tools generated. Check config file and structure.")
