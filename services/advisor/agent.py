...
from services.app_settings import config_manager, ConfigManager
...
class AgentAdvisor:
        ...
        try:
            ...
            for tool_config in agent_config.tools:
                ...
                if tool_name:
                    ...
                    logger.debug(
                        f"Loaded tool config: {tool_name} (type: {tool_config.type})"
                    )
    ...
    def _load_tools_from_config(self) -> List:
        ...
        for tool_name, tool_config in self.tool_configs.items():
            ...
            if tool_instance:
                ...
                logger.info(
                    f"✅ Loaded tool: {tool_name} (type: {tool_config.get('type')})"
                )
            else:
    ...
    def initialize(self):
        ...
        try:
            ...
            if not api_key:
                logger.error(
                    "GROQ_API_KEY not found in config. Agent cannot be initialized."
                )
                return False
            ...
            if not self.tools:
                logger.warning(
                    "No tools loaded from configuration. Agent may not function properly."
                )
            ...
            self.agent = create_react_agent(
                model=self.llm, tools=self.tools, prompt=self.prompt
            )
    ...
    def invoke(
            self, messages: Union[List[Dict[str, str]], Dict[str, List[Dict[str, str]]]]
    ):
        """
        ...
        if not self.is_initialized or self.agent is None:
            ...
            if not self.initialize():
                return {
                    "output": "Agent could not be initialized. Please check the logs."
                }
        ...
        try:
            ...
            if integration_manager.is_langsmith_configured:
                ...
                if (
                    ...
                    user_msg = next(
                        (m for m in messages["messages"] if m.get("role") == "user"),
                        None,
                    )
                    if user_msg and "content" in user_msg:
                        metadata["user_query"] = user_msg["content"][
                                                 :100
                                                 ]  # First 100 chars
        ...
        if tool_name:
            if tool_name in self.tool_configs:
                return {
                    ...
                    "status": (
                        "loaded" if tool_name in self.tool_instances else "not_loaded"
                    ),
                }
            else:
...
def _test_agent():
    config_manager = ConfigManager(
        storage_file="/Users/phamthanh/Desktop/ai/zalo_bot/data/app_config.json")
    try:
        asyncio.run(config_manager.load())
    except RuntimeError:
        pass
    advisor = AgentAdvisor()
    result = advisor.invoke("Mô tả yêu cầu hệ thống của tôi")
    print(result)
